...
if (isset($_GET['id']) and isset($_GET['image_name'])) {
    ...
    if ($image_name != "") {
        // deleting image
        $path = "images/course/" . $image_name; //getting  path for the image to be deleted
        $remove = unlink($path); //removing image from that path

        // checking if image is removed
        if ($remove == false) {
            // session message
            $_SESSION['remove'] = "<div class='error'>failed to remove course image</div>";
    ...
    if ($res == true) {
        $_SESSION['delete'] = "<div class='success'>course is deleted successfully</div>"; // creating delete session with appropreate msg
